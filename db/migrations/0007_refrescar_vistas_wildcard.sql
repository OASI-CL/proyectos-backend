-- Recrea v_permisos, v_proyectos y v_permisos_comite para que recojan las
-- columnas agregadas en 0005 (n_catastro, en_universo_permisos, tipologia_id,
-- comite_registro/fecha_actualizacion/quien_actualizo, etc.).
--
-- POR QUÉ HACE FALTA: estas tres vistas usan `pr.*` / `p.*` (todas las
-- columnas de proyectos/permisos). Postgres resuelve ese wildcard UNA VEZ, al
-- crear la vista — agregar una columna después con ALTER TABLE no la agrega
-- sola a una vista que ya existía. Sin este refresh, cualquier columna nueva
-- de proyectos/permisos queda invisible para la API aunque esté en la tabla.
--
-- Es exactamente el mismo texto que las definiciones de
-- 0002_triggers_vistas_catalogos.sql / 0003 (para v_permisos y v_proyectos) y
-- 0002_comite_acumulado (para v_permisos_comite) — no cambia ninguna lógica,
-- solo se re-crean para que el `*` se re-evalúe contra las columnas de hoy.
--
-- LECCIÓN: cualquier migración que agregue una columna a `proyectos` o
-- `permisos` tiene que terminar con este mismo refresh, o la columna nueva
-- queda enterrada en la tabla sin que la API la vea nunca.
