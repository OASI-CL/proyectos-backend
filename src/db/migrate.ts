 ...
 *   local   npm run migrate                 (scripts/migrate.ts)
 *   AWS     the db-ops Lambda, action=migrate (src/ops/dbOps.ts), which CI
 *           invokes after every deploy
 *
 ...
 *   baseline     tables exist but no schema_migrations -> a database created
 ...
 *     schema_migrations row in one transaction, so a failure leaves nothing
...
export async function runMigrations(pool: Pool, log: (msg: string) => void = console.log): Promise<MigrationResult> {
  ...
  try {
    ...
    const hasTracking = await tableExists(client, 'public.schema_migrations')
